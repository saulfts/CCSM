...
abstract class K2ModelItemlistExt extends K2Model
{

	public static function getData($ordering = NULL)
	{
		...
		switch ($task)
		{
			...
			case 'category' :
				...
				{
					// $categories = K2ModelItemlistExt::getCategoryTree($id);
					$categories = K2ModelItemlistExt::getCategoryTree($id);
					$sql = @implode(',', $categories);
					$query .= " AND c.id IN ({$sql})";
				}
				
				$tag = JRequest::getString('tagid');
				if ($tag != ''){
					$query .= " AND i.id IN (SELECT itemID FROM #__k2_tags_xref WHERE tagID=".(int)$tag.")";
				}
			...
			case 'search' :
				...
				$sql = K2ModelItemlistExt::prepareSearch($search);
				if (!empty($sql))
				{
					$query .= $sql;
				}
			...
			default :
				...
				if (is_array($searchIDs) && count($searchIDs))
				{
					...
					else
					{
						...
						$result = K2ModelItemlistExt::getCategoryTree($searchIDs);
						if (count($result))
						{
							$sql = @implode(',', $result);
							$query .= " AND c.id IN ({$sql})";
	...
	public static function getCategoryTree($categories, $associations = false)
	{
		$mainframe = JFactory::getApplication();
		$db = JFactory::getDBO();
		$user = JFactory::getUser();
	...
	public static function getCategoryChildren($catid, $clear = false)
	{
		...
		foreach ($rows as $row)
		{
			...
			if (K2ModelItemlistExt::hasChildren($row->id))
			{
				K2ModelItemlistExt::getCategoryChildren($row->id);
			}
		}
		return $array;
	}
	...
	public static function getCategoryFirstChildren($catid, $ordering = NULL)
	{
	...
	public static function getAuthorLatest($itemID, $limit, $userID)
	{
	...
	public static function getRelatedItems($itemID, $tags, $params)
	{
	...
	public static function getModuleItems($moduleID)
	{
	...
	public function getCategoriesTree()
	{
		...
		$tree = array();
		return K2ModelItemlistExt::buildTree($categories);
	}
	...
	public function buildTree(array &$categories, $parent = 0)
	{
		...
		foreach ($categories as &$category)
		{
			...
			{
				$children = K2ModelItemlistExt::buildTree($categories, $category->id);
				if ($children)
				{
	...
	public function getTreePath($tree, $id)
	{
		...
		else
		{
			foreach ($tree as $key => $root)
			{
				...
				{
					$retry = K2ModelItemlistExt::getTreePath($root->children, $id);
