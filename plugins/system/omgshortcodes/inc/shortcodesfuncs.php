...
class Shortcodes_Handlers
{
	...
	public static function collapse($atts, $content = null)
	{
		...
		$html .= '<div id="' . $parent . '" class="panel-group'
				. ((trim($class) != '') ? ' ' . htmlspecialchars(trim($class)) : '')
				. '">'
					.Shortcodes_Parser::instance()->render($stripedContent)
				. '</div>
	...
	public static function collapse_item($atts, $content = null)
	{
		...
		// var_dump($atts);
		$ci_id = self::randomkeys(4);

		$collapse_item = '<div class="panel panel-' . $type . '">'
					. '<div class="panel-heading">'
					. '<h4 class="panel-title">'
					. '<a href="#icollapse-' . $ci_id . '" data-parent="#' . $parent . '" data-toggle="collapse" class="'.($active=='active'?'':'collapsed').'">'
	...
	public static function googlefont($atts, $content = null){
		...
		if($align!=''||$margin!='')
		{
			$roundb = '<div style="';
			if($align!='')
				$roundb .= 'text-align:' . $align . ';';
			if($margin!='')
				$roundb .= 'margin:' . $margin . ';';
			$roundb .= '">';
		}
		$rounda = '';
		if($align!=''||$margin!='')
	...
	public static function list_item($atts, $content = null ){
		...
		if($list_color!=''){
			// return '<li><i class="fa fa-' . $list_type . '"></i><span>' . $content . '</span></li>';
		}else{
			// return '<li><i class="fa fa-' . $list_type . '"></i>' . $content . '</li>';
		}
		$li_i = '';
		$li_i = '<li>'
			// . ($list_color!='' ? '<span>' : '')
			. ($list_type!='' ? '<i class="fa fa-' . $list_type . '"></i>' : '')
			. $content
			// . ($list_color!='' ? '</span>' : '')
			. '</li>';
		return $li_i;
	...
	{	
		...
		if (trim($id) == "") $id = self::randomkeys(8);
	...
	public static function plan($atts, $content = null ){
		...
		return  '<div class="column text-center col-sm-' . round(12/$pcolumns) . (('true' == strtolower($featured)) ? ' featured' : '') .'">' .
	...
	public static function tabs($atts, $content)
	{
		...
		$cls = (trim($class) != '') ? ' class="'.htmlspecialchars(trim($class)).'"' : '';
		$style = ((trim($max_width) != '' && intval($max_width) != 0) ? ' style="max-width:'. trim($max_width) . ';"' : '');
		$html .= '
			<div id="tabs-'.$add_id.'"' . $cls . $style .'>
	...
	{	
		...
		$tabsArr[] = array("title" => $title , "content" => Shortcodes_Parser::instance()->render($content));
		unset($title);
	}
	...
	public static function testimonial($atts, $content = null){
		...
		$testimonial_avatar .= '<small class="testimonial-author text-right">';
